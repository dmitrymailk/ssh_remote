...
```
### 4. Если все еще просит пароль от сервера
#### 4.1 Ошибка с правами доступа к папке
1. Для этого заходим на сервер и вводим руками пароль
```bash
ssh root@94.198.216.62 -i ~/.ssh/lesson_ssh
```

2. Меняем права доступа для папки ~/.ssh/

- Это значит что мы сделали папку только для чтения и записи ее владельцем
